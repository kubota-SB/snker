...
(() => {
  ...
  const state = { all: [], filtered: [], group: '2026', shown: PAGE_SIZE, query: '' };
  ...
  async function init() {
    Object.assign(els, {
      yearNav: $('#yearNav'), grid: $('#sneakerGrid'), loadMore: $('#loadMoreBtn'), loading: $('#loadingState'), empty: $('#emptyState'),
  ...
  function bindGlobalEvents() {
    els.search.addEventListener('input', () => { state.query = els.search.value.trim(); state.shown = PAGE_SIZE; applyFilter(true); });
    els.clear.addEventListener('click', () => { els.search.value=''; state.query=''; state.shown=PAGE_SIZE; applyFilter(true); els.search.focus(); });
    els.loadMore.addEventListener('click', () => { state.shown += PAGE_SIZE; renderGrid(false); });
  ...
  function selectGroup(group) {
    state.group=group; state.query=''; state.shown=PAGE_SIZE; els.search.value='';
  ...
  function renderGrid(reset=false) {
    ...
    const rendered=els.grid.children.length;
    const target=Math.min(state.shown,state.filtered.length);
    if(rendered>target || reset) {
      els.grid.innerHTML='';
      state.filtered.slice(0,target).forEach((item,i)=>els.grid.appendChild(makeCard(item,i)));
    } else {
      state.filtered.slice(rendered,target).forEach((item,i)=>els.grid.appendChild(makeCard(item,rendered+i)));
    }
    els.empty.hidden=state.filtered.length!==0;
    els.loadMore.hidden=target>=state.filtered.length;
    if(!els.loadMore.hidden) els.loadMore.textContent=`もっと見る（残り ${state.filtered.length-target}）`;
  }
  ...
  function detailMarkup(item){return `
    <div class="detail-grid">
      <div class="detail-media">
        ...
        <div class="viewer-actions">${hasStockx(item)?'<button class="viewer-load" type="button">360°ビューを読み込む</button>':''}<span class="viewer-status">${hasStockx(item)?'必要な時だけ36枚を読み込みます':'StockX 360°素材なし'}</span></div>
      </div>
  ...
  async function load360(item,button){
    const status=$('.viewer-status',els.detail);button.disabled=true;button.textContent='360°素材を確認中…';status.textContent='StockXの360°画像を探しています';
    for(const folder of folderCandidates(item)){for(const level of ['Lv2','Lv1']){if(!await probeImage(frameUrl(folder,1,level),3000))continue;if(!await probeImage(frameUrl(folder,18,level),3000))continue;viewer={folder,level,frame:1};enableViewer();button.remove();status.textContent='左右ドラッグ / スワイプ / 矢印で回転';return;}}
    button.disabled=false;button.textContent='360°ビューを再確認';status.textContent='このモデルの360°素材を確認できませんでした';
  }
  ...
  function marketMarkup(item){const sn=getLink(item,'snkrdunk'),sx=getLink(item,'stockx');return `<details class="market-section"><summary>価格チャート － サイズ・新品 / 中古を比較</summary><div class="market-inner"><p class="market-note">チャートを開いた時だけ外部の公開ページを確認します。取得制限時は公式ページへのリンクを利用してください。</p><div class="market-grid">${marketPanel('snkrdunk',sn,item.sku)}${marketPanel('stockx',sx,item.sku)}</div></div></details>`;}
  function marketPanel(source,url,sku){const name=source==='stockx'?'StockX':'SNKRDUNK', unit=source==='stockx'?'US':'CM', sizes=source==='stockx'?['All','3.5','4','4.5','5','5.5','6','6.5','7','7.5','8','8.5','9','9.5','10','10.5','11','11.5','12','12.5','13','14','15','16','17']:['All','22.5','23','23.5','24','24.5','25','25.5','26','26.5','27','27.5','28','28.5','29','29.5','30','30.5','31','31.5','32'], def=source==='stockx'?'9':'27';const opts=sizes.map(x=>`<option ${x===def?'selected':''} value="${x}">${x}</option>`).join('');return `<section class="market-source ${source==='stockx'?'market-stockx':'market-snkr'}" data-source="${source}" data-url="${escapeAttr(url)}" data-sku="${escapeAttr(sku)}"><div class="market-source-head"><strong>${name}</strong><span class="market-status">未取得</span></div><div class="market-controls"><label>サイズ (${unit}) <select class="market-size">${opts}</select></label><div class="market-condition"><button type="button" class="active" data-condition="new">新品</button><button type="button" data-condition="used">中古</button></div></div><div class="market-chart-box"><div class="market-empty">チャートを開くと読み込みます</div><svg class="market-chart" hidden viewBox="0 0 520 190"></svg></div><div class="market-metrics"></div><div class="market-actions"><button class="market-load" type="button">価格を読み込む</button>${url?`<a class="market-open" href="${escapeAttr(url)}" target="_blank" rel="noopener">${name}で確認</a>`:''}</div></section>`;}
  function escapeAttr(s){return String(s||'').replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}

  function marketClickHandler(e){const cond=e.target.closest('.market-condition button');if(cond){const p=cond.closest('.market-source');$$('.market-condition button',p).forEach(b=>b.classList.toggle('active',b===cond));if(p.dataset.loadedKey)loadMarket(p,true);return;}const load=e.target.closest('.market-load');if(load)loadMarket(load.closest('.market-source'),true);}
  function marketChangeHandler(e){if(e.target.matches('.market-size')){const p=e.target.closest('.market-source');if(p.dataset.loadedKey)loadMarket(p,true);}}
  ...
  async function fetchText(target){const key='reader:'+target;if(marketCache.has(key))return marketCache.get(key);const c=new AbortController(),t=setTimeout(()=>c.abort(),15000);const p=fetch(readerUrl(target),{signal:c.signal,cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('HTTP '+r.status);return r.text()}).finally(()=>clearTimeout(t));marketCache.set(key,p);return p;}
  ...
  async function loadMarket(panel,force=false){const raw=panel.dataset.url,status=$('.market-status',panel),empty=$('.market-empty',panel),svg=$('.market-chart',panel);if(!raw){status.textContent='リンクなし';status.className='market-status error';empty.textContent='このサイトの商品リンクが登録されていません。';return;}const source=panel.dataset.source,size=$('.market-size',panel).value,condition=$('.market-condition button.active',panel)?.dataset.condition||'new',target=withParams(raw,source,size,condition);$('.market-open',panel)?.setAttribute('href',target);const key=`${source}|${target}|${condition}`;if(!force&&panel.dataset.loadedKey===key)return;status.textContent='取得中';status.className='market-status';empty.hidden=false;empty.textContent='価格情報を取得しています…';svg.hidden=true;try{const text=await fetchText(target),data=source==='stockx'?stockxData(text,condition):snkrData(text);drawChart(panel,data);panel.dataset.loadedKey=key;status.textContent=data.points?.length>=2?'表示中':'一部取得';status.className='market-status '+(data.points?.length>=2?'ok':'warn');}catch(err){status.textContent='取得不可';status.className='market-status error';empty.hidden=false;empty.textContent='外部ページの取得制限でチャートを読み込めませんでした。公式ページで確認してください。';svg.hidden=true;}}
